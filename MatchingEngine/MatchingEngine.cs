...
{
    // Minimum score to accept a match (synonym hints lift good pairs; too low steals wrong paths)
    private const double MinThreshold = 0.175;
        ...
        List<Field> amazonFields)
    {
        ...
        foreach (var amazon in sortedAmazon)
        {
            ...
            AmazonMatchingHints.AppendHintTokens(amazon.Name, aTokens);
        ...
        Field amazon, Field quipt, double specificity)
    {
        ...
        // ── 7. Domain hints: light path alignment (kept small to avoid “stealing” the right Quipt XPath)
        score += AmazonMatchingHints.PathAlignmentBonus(amazon.Name, quipt) * 0.55;

        // ── 8. Specificity adjustment ──
